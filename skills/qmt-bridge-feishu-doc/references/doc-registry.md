...
```
QMT Trading Skill/                   # FEISHU_QMT_BRIDGE_FOLDER_TOKEN（根目录，可选）
...
|------|-------------|----------|---------------|----------------|
| `daily-eval` | 每日复盘 | `QMT Trading Skill 当日复盘` | `reports/feishu_daily_eval.md`（`daily_trade_report.py --feishu-md`） | execution-review |
| `daily-pnl` | 盈亏快照 | `QMT Trading Skill 当日盈亏` | `reports/feishu_daily_pnl.md` | daily-pnl |
| `return-analysis` | 涨跌分析 | `QMT Trading Skill 涨跌分析` | `reports/feishu_return_analysis.md` | return-analysis |
| `portfolio-risk` | 组合风控 | `QMT Trading Skill 组合风险` | `reports/feishu_portfolio_risk.md` | portfolio-risk |
...
| 字段 | 说明 | 示例 |
|------|------|------|
| 标题前缀 | 上表固定字符串 | `QMT Trading Skill 当日复盘` |
| 主题 | 交易日 `YYYY-MM-DD`，或分析范围 | `2026-05-21` / `持仓` |
| 同步时刻 | 上传时分秒；与交易日同日只写一次日期 | `15:30:00` |

示例：`QMT Trading Skill 当日复盘 2026-05-21 15:30:00`
...
1. 环境变量 `FEISHU_DAILY_EVAL_DOC_ID` 等（滚动更新用）  
...
| 键 / 环境变量 | 说明 |
|---------------|------|
| `daily-eval-wiki-parent` | 仅当用户**未**在对话指定父文档、且希望长期固定父节点时使用 |
| `FEISHU_DAILY_EVAL_WIKI_PARENT_TOKEN` | 同上 |

**默认**：不传父节点 → `wiki +node-create` 落在个人知识库根。用户当场给的 wiki URL **优先**于上述配置。
